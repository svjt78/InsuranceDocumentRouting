...
function BucketMappingRow({ mapping, departmentData, onSave, onDelete, onUpdate }) {
  // Initialize state using the backend's snake_case key
  ...
  useEffect(() => {
    // When state changes, call onUpdate with keys that match the backend.
...
function EmailSettingsRow({ setting, departmentData, onSave, onDelete, onUpdate }) {
  // Initialize state using email_addresses from backend.
  ...
  useEffect(() => {
    // When state changes, update using backend keys.
...
export default function Configuration() {
  // Dynamic department hierarchy fetched from the /lookup endpoint.
  ...
  // Fetch document hierarchy (organizational hierarchy) from /lookup endpoint.
  useEffect(() => {
    ...
    try {
      if (rowData.isNew) {
        const res = await fetch("http://localhost:8000/bucket-mappings", {
          ...
          body: JSON.stringify({
            bucket_name: rowData.bucketName,
  ...
  const handleAddBucketMapping = () => {
    const newMapping = {
      ...
      bucketName: "", // local state uses bucketName; payload conversion happens on save
      department: "",
      category: "",
      subcategory: "",
      isNew: true,
    ...
    try {
      if (rowData.isNew) {
        const res = await fetch("http://localhost:8000/email-settings", {
          ...
          body: JSON.stringify({
            department: rowData.department,
            email_addresses: rowData.emails,
          }),
        });
        if (!res.ok) throw new Error("POST failed");
        const savedRow = await res.json();
  ...
  const handleAddEmailSetting = () => {
    const newSetting = {
      id: Date.now(),
      department: "",
      emails: "", // local state uses emails; conversion happens on save
      isNew: true,
    };
